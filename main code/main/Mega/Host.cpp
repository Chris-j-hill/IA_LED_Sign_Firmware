...
#define space_colon_string " : "
#define dash_space_string "- "
#define tab_string  "\t"
#define space_string  " "
#define percent_string  "%"
#define divide_string  "/"
#define comma_space_string ", "
...
inline void space() {
  Serial.print(F(space_string));
};

inline void tab() {
  Serial.print(F(tab_string));
};

inline void space_colon() {
  Serial.print(F(space_colon_string));
};

inline void dash_space() {
  Serial.print(F(dash_space_string));
};

inline void comma_space() {
  Serial.print(F(comma_space_string));
}

inline void yes() {
  Serial.print(F(yes_string));
}

inline void no() {
  Serial.print(F(no_string));
}

inline void y() {
  Serial.print(F(y_string));
}
inline void n() {
  Serial.print(F(n_string));
}
...
byte Host::data_set_LUT(byte data_set[6]) {

  if ((strncmp(data_set, "stop", 4) == 0))
    return STOP_REPORT;

  else if ((strncmp(data_set, "menu", 4) == 0))
    return REPORT_MENU;

  else if ((strncmp(data_set, "config", 6) == 0))
    return REPORT_CONFIG;

  //check substring for text keyword
  byte text_str[4];
