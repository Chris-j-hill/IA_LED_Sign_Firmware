...
void RGBmatrixPanel::init(uint8_t rows, uint8_t a, uint8_t b, uint8_t c,
  uint8_t sclk, uint8_t latch, uint8_t oe1, boolean dbuf, uint8_t width) {
  ...
  oe1port    = portOutputRegister(digitalPinToPort(oe1));
  oe1pin     = digitalPinToBitMask(oe1);
...
RGBmatrixPanel::RGBmatrixPanel(
  ...
  uint8_t sclk, uint8_t latch, uint8_t oe1, boolean dbuf) :
  Adafruit_GFX(32, 16) {

  init(8, a, b, c, sclk, latch, oe1, dbuf, 32);
}
...
RGBmatrixPanel::RGBmatrixPanel(
  ...
  init(16, a, b, c, sclk, latch, oe1, dbuf, width);
...
// Constructor for 32x32 or 32x64 2 panel display:
RGBmatrixPanel::RGBmatrixPanel(
  uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t sclk, uint8_t latch, uint8_t oe1, 
  uint8_t oe2, boolean dbuf, uint8_t width, uint8_t mode) :
  Adafruit_GFX(width, 32) {
  ...
  addrdpin  = digitalPinToBitMask(d);
  _oe2      = oe2;
  oe2port   = portOutputRegister(digitalPinToPort(oe2));
  oe2pin    = digitalPinToBitMask(oe2);
  _mode     = mode;
  twoscreens= true;
}
...
void RGBmatrixPanel::begin(void) {
  ...
  pinMode(_oe1  , OUTPUT); *oe1port   |= oe1pin;    // High (disable output)
  ...
  if(twoscreens) {
	pinMode(_oe2, OUTPUT); *oe2port   |= oe2pin;   // High (disable output)
	Mode(_mode);
  }
...
void RGBmatrixPanel::Mode(uint8_t mode) {
  _mode = mode;
...
#define LOOPTIME     700  // Actual value measured = 188
...
void RGBmatrixPanel::updateDisplay(void) {
  ...
  if(oe1flag)
  {
	*oe1port  |= oe1pin;  // Disable screen1 output during row/plane switchover
  }	  
  if(oe2flag)
  {
	*oe2port  |= oe2pin;  // Disable screen2 output during row/plane switchover
  }
  *latport |= latpin; // Latch data loaded during *prior* interrupt
  ...
  if(oe1flag)
  {
    *oe1port  &= ~oe1pin;   // Re-enable output 1
  }
  else
  {
    *oe1port  |= oe1pin;
  ...
  if(oe2flag)
  {
    *oe2port  &= ~oe2pin;   // Re-enable output 2
  }
  else
  {
    *oe2port  |= oe2pin;
