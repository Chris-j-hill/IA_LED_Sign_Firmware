//#ifndef Due_Class_CPP
//#define Due_Class_CPP 
//
...
//  int external_sd_card_initialised = 0;
//  int internal_sd_card_initialised = 0;
//
...
//  int theta = beta;
//  int alpha = 0;
//  while (millis() < beta + 40000 && external_sd_card_initialised == 0) {    //wait for 40 seconds or until card inserted
//    if (external_sd_card.begin(SD1_CS)) {    //if card initialised sucessfully exit wait loop
//      external_sd_card_initialised = 1;              //loop exit conditions if card detected
//    }
...
//  }
//
//  if (external_sd_card_initialised == 0) {
...
//  beta = millis();
//  external_sd_card_initialised = 0;
//  alpha = 0;
//
//  while (millis() < beta + 5000 && external_sd_card_initialised == 0) {    // make sure directory exists on external card, may take a moment, if longer than 5 seconds then error
//
...
//    }
//    else
//      external_sd_card_initialised = 1;    //folder exists, exit loop
//  }
//  if (external_sd_card_initialised == 0) {
...
//      internal_sd_card_initialised = 1;              //loop exit conditions if card detected
//    }
//    else {
//      if (alpha % 100 == 0)
...
//  }
//
//  if (internal_sd_card_initialised == 0)
//    external_sd_card.initError("internal_sd_card:");    //stops code when called
//
...
//  if (!external_sd_card.chdir(sd_ext_dir)) external_sd_card.errorExit("external_sd_card.chdir");
//
//  // make /SIGN2 the default directory for internal_sd_card
//  if (!internal_sd_card.chdir(sd_int_dir)) internal_sd_card.errorExit("internal_sd_card.chdir");
//
...
//    if (n == 0) break;
//    if (file2.write(buf, n) != n) internal_sd_card.errorExit("error writing internal file");
